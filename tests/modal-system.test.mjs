...
const spatial = readFileSync(new URL('../js/nodes/spatial.js', import.meta.url), 'utf8');
...
test('String Formatter usa el contrato modal de Desktop y persiste JSON', () => {
    ...
    assert.match(attributes, /data-schema-action="splitter-open-editor"/);
    assert.match(attributes, /data-schema-action="list-exploder-open-editor"/);
    assert.match(attributes, /data-schema-action="strrep-open-editor"/);
    assert.match(index, /id="string-replacer-editor-modal"/);
    assert.match(schema, /openStringReplacerEditor/);
    assert.match(attributes, /data-schema-action="aggregator-open-editor"/);
    assert.match(attributes, /data-schema-action="attr-manager-open-editor"/);
    assert.match(attributes, /data-schema-action="attr-manager-v2-open-editor"/);
    assert.match(index, /id="aggregator-editor-modal"/);
    ...
    assert.match(spatial, /sp_anchored_snapper:[\s\S]+data-schema-action="geom-transform-open-editor"/);
    assert.match(spatial, /sp_neighbor_finder:[\s\S]+data-schema-action="geom-transform-open-editor"/);
    assert.match(spatial, /sp_spatial_relator:[\s\S]+data-schema-action="geom-transform-open-editor"/);
    ...
    assert.match(index, /id="reader-editor-file-input"[^>]+type="file"/);
    assert.match(index, /Cargar datos desde el dispositivo/);
    assert.match(schema, /JETLReaderFileStore/);
    assert.doesNotMatch(schema, /reader-select-files[^\n]+\.click\(/);
    assert.match(schema, /reader-refresh-preview/);
    assert.match(index, /data-ui-action="reader-add-row"[\s\S]+data-ui-action="reader-apply-edits"[\s\S]+data-ui-action="reader-discard-edits"/);
    assert.match(readers, /df-reader-edited-data/);
    assert.match(schema, /function applyReaderEditorChanges\(\)/);
    assert.match(schema, /data-reader-row-index/);
